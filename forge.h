...
extern void f_quaternion_to_rotationd(double *rotation, double x, double y, double z, double w); /* Convert a quaternion in to a Radian Roll, pitch and yaw.*/
extern void f_quaternion_to_rotationf(float *rotation, float x, float y, float z, float w); /* Convert a quaternion in to a Radian Roll, pitch and yaw.*/
...
extern void f_rgb_to_xyz(float *xyz, float r, float g, float b); /* Converts from sRBG to CIE XYZ.*/
extern void f_xyz_to_rgb(float *rgb, float x, float y, float z); /* Converts from CIE XYZ to sRGB.*/

extern void f_aces_to_xyz(float *xyz, float r, float g, float b); /* Converts from ACES2065-1 RBG to CIE XYZ.*/
extern void f_xyz_to_aces(float *rgb, float x, float y, float z); /* Converts from CIE RBG to ACES2065-1 RBG.*/

extern void f_xyz_to_lab(float *lab, float x, float y, float z); /* Converts from CIE XYZ to LAB.*/
extern void f_lab_to_xyz(float *xyz, float l, float a, float b); /* Converts from LAB to CIE XYZ.*/

extern void f_rgb_to_lab(float *lab, float r, float g, float b); /* Converts from sRBG to LAB.*/
extern void f_lab_to_rgb(float *rgb, float l, float a, float b); /* Converts from LAB to sRGB.*/

extern void f_xyz_to_primaries(float *output, float x, float y, float z, unsigned int prime_count, float *prime_coordinates); /* Converts CIE XYZ to an arbitrary number of primaties. */
...
extern int f_shape_inside2df(const float *array, uint point_count, float x, float y); /* Tests if apoint is inside a closed 2D 32 bit float n-gon. */
...
extern boolean	f_text_filter_case_insensitive(char *text, char *filter);
extern uint		f_text_sort(char *text_a, char *text_b);
