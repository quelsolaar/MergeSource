...
typedef struct{
	...
	void *user_data; /* user pointer that will be given to all above function pointers when called. */
	void *library; /* internal handle for the loaded library, so that it can be unloaded. */
}HxAUPIPlugin;

extern HxALibExport void hxa_upi_plugin_library_initialize(void (*hxa_upi_library_register_func)(HxAUPIPlugin *plugin));
